<template>
  ...
  >
    <template v-if="columnConfig.children">
      <xColumn
        v-for="(configItem, configItemIndex) in columnConfig.children"
        :key="configItem.name || configItemIndex"
    ...
    <template v-if="!columnConfig.children" slot="header">
      {{ columnConfig.label }}
      <el-tooltip
        v-if="columnConfig.tooltip"
        :effect="tooltipConfig.effect"
        :placement="tooltipConfig.placement"
      >
        <!-- <template #content> <span v-html="columnConfig.tooltip" /> </template> -->
        <template #content>
          <div><span v-html="columnConfig.tooltip" /></div>
        </template>
