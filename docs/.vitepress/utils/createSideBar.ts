...
import { rootDir, rootDirFn, docsDirFullPathLen } from '../../scripts/globalConfig'
...
export function getSidebarData(sidebarGenerateConfig: SidebarGenerateConfig = {}) {
  ...
  // 获取目录的绝对路径
  const dirFullPath = !sidebarGenerateConfig?.dirName ? rootDir : rootDirFn(sidebarGenerateConfig.dirName)
...
function getNavDataArr(dirFullPath: string, level: number, maxLevel: number): DefaultTheme.NavItem[] {
  // 获取所有文件名和目录名
  const allDirAndFileNameArr = readdirSync(dirFullPath)
  // console.log(allDirAndFileNameArr,'67890')
